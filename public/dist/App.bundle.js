...
var autocomplete = function autocomplete(addressInputElem, latInputElem, lngInputElem) {
  if (!addressInputElem) return;

  var dropdown = new google.maps.places.Autocomplete(addressInputElem);
  ...
  addressInputElem.on('keydown', function (e) {
...
var getSearchResultsItems = function getSearchResultsItems(stores) {
...
var typeAhead = function typeAhead(searchElement) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      if (res.data.length) return searchResults.innerHTML = _dompurify2.default.sanitaze(getSearchResultsItems(res.data));
      searchResults.innerHTML = '<div class="search__result">No results for ' + _dompurify2.default.sanitaze(_this.value) + '</div>';
    }).catch(function (err) {
  ...
  searchInput.on('keyup', function (e) {
    ...
    var current = searchElement.querySelector('.' + activeClass);
    var items = searchElement.querySelectorAll('.search__result');
    var next = void 0;
